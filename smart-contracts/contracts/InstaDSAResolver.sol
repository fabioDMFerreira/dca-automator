...
contract Helpers {
    ...
    function getContractCode(address _addr)
        public
        view
...
contract AccountResolver is Helpers {
    ...
    function getAuthorityIDs(address authority)
        public
        view
        returns (uint64[] memory)
    {
        ListInterface.UserLink memory userLink =
            listContract.userLink(authority);
        uint64[] memory IDs = new uint64[](userLink.count);
        uint64 id = userLink.first;
        for (uint256 i = 0; i < userLink.count; i++) {
            IDs[i] = id;
            ListInterface.UserList memory userList =
                listContract.userList(authority, id);
            id = userList.next;
        }
        return IDs;
    }

    function getAuthorityAccounts(address authority)
        public
        view
    ...
    function getIDAuthorities(uint256 id)
        public
        view
        returns (address[] memory)
    {
        ListInterface.AccountLink memory accountLink =
            listContract.accountLink(uint64(id));
        ...
        for (uint256 i = 0; i < accountLink.count; i++) {
            ...
            ListInterface.AccountList memory accountList =
                listContract.accountList(uint64(id), authority);
            authority = accountList.next;
        }
        return authorities;
    }

    function getAccountAuthorities(address account)
        public
        view
        returns (address[] memory)
    {
    ...
    function getAccountVersions(address[] memory accounts)
        public
        view
    ...
    function getAuthorityDetails(address authority)
        public
        view
        returns (AuthorityData memory)
    {
    ...
    function getAccountIdDetails(uint256 id)
        public
        view
    ...
    function getAuthorityTypes(address[] memory authorities)
        public
        view
        returns (AuthType[] memory)
    {
        ...
        for (uint256 i = 0; i < authorities.length; i++) {
            ...
            for (uint256 k = 0; k < gnosisFactoryContracts.length; k++) {
                bytes memory multiSigCode =
                    gnosisFactoryContracts[k].proxyRuntimeCode();
                if (
                    keccak256(abi.encode(multiSigCode)) ==
                    keccak256(abi.encode(_contractCode))
                ) {
                    isSafe = true;
                }
            }
    ...
    function getAccountAuthoritiesTypes(address account)
        public
        view
        returns (AuthType[] memory)
    {
...
contract ConnectorsResolver is AccountResolver {
    ...
    function getEnabledConnectorsData()
        public
        view
        returns (ConnectorsData[] memory)
    {
        uint256 enabledCount = connectorsContract.connectorCount();
        ConnectorsData[] memory connectorsData =
            new ConnectorsData[](enabledCount);
        uint256 connectorArrayLength = connectorsContract.connectorLength();
        uint256 count;
        for (uint256 i = 0; i < connectorArrayLength; i++) {
            address connector = connectorsContract.connectorArray(i);
    ...
    function getStaticConnectors() public view returns (address[] memory) {
        ...
        for (uint256 i = 0; i < staticLength; i++) {
            staticConnectorArray[i] = connectorsContract.staticConnectorArray(
                i
            );
        }
    ...
    function getStaticConnectorsData()
        public
        view
        returns (ConnectorsData[] memory)
    {
        uint256 staticLength = connectorsContract.staticConnectorLength();
        ConnectorsData[] memory staticConnectorsData =
            new ConnectorsData[](staticLength);
        for (uint256 i = 0; i < staticLength; i++) {
            address staticConnector =
                connectorsContract.staticConnectorArray(i);
